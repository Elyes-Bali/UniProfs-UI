...
import { usePaymentStore } from "../store/paymentStore"; // 🔥 Import the new store
...
const PlansPage = () => {
  // 🔥 Destructure the store action and state
  const { subscribeToPlan, isLoading } = usePaymentStore();

  // Removed the local subscribeToPlan function
                  ...
                  <motion.button
                    onClick={() =>
                      plan.isFree
                        ...
                        : subscribeToPlan(plan.name, plan.price) // 🔥 Call the store action
                    }
                    disabled={isLoading} // 💡 Use store loading state to prevent double clicks
